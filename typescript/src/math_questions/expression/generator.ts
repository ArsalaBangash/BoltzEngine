...
import { BoundedExpr, Expr, ExprToken, Operator, SubExprLocation, SubExprToken, TokenType } from './models';
...
export const getChallengeTokens = (operators: Operator[], diff: Difficulty): ExprToken[] => {
  const ops = [...operators];
  ...
  expr.push(...getExprTokens({ diff, isBounded: false, op: currentOp, subExprLocation }));
