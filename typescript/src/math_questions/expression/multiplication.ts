...
const _getZeroBoundTokens = (expr: Expr): ExprToken[] => {
  ...
  const randomValue: number = positiveRandomNum(difficultyBounds[expr.diff].max);
  switch (expr.subExprLocation) {
    case SubExprLocation.NEITHER: {
      const leftToken: ExprToken = { type: TokenType.Number, value: randomValue };
    ...
    case SubExprLocation.BOTH: {
      const leftToken: SubExprToken = { type: TokenType.SubExpr, bounded: true, value: randomValue };
      const rightToken: SubExprToken = { type: TokenType.SubExpr, bounded: true, value: 0 };
      return tokens.concat([leftToken, rightToken]);
    }
...
const _getNoSubExprTokens = (expr: Expr): ExprToken[] => {
  const tokens: ExprToken[] = [];
  if (expr.isBounded) {
    const multiplesForBound: Multiples[] = _getMultiplesForBound((expr as BoundedExpr).bound);
    const multiples = multiplesForBound[randomNum(0, difficultyBounds[expr.diff].max)];
    const leftToken: ExprToken = { type: TokenType.Number, value: multiples.left };
    const rightToken: ExprToken = { type: TokenType.Number, value: multiples.right };
    return tokens.concat([leftToken, rightToken]);
  } else {
    const max: number = difficultyBounds[expr.diff].max;
    const leftToken: ExprToken = { type: TokenType.Number, value: positiveRandomNum(max) + 1 };
    const rightToken: ExprToken = { type: TokenType.Number, value: positiveRandomNum(max) + 1 };
    return tokens.concat([leftToken, rightToken]);
  }
};
...
const _getBothSubExprTokens = (expr: Expr): ExprToken[] => {
  ...
  if (expr.isBounded) {
    const multiplesForBound: Multiples[] = _getMultiplesForBound((expr as BoundedExpr).bound);
    const multiples = multiplesForBound[randomNum(0, difficultyBounds[expr.diff].max)];
    const leftToken: SubExprToken = { type: TokenType.SubExpr, bounded: true, value: multiples.left };
    const rightToken: SubExprToken = { type: TokenType.SubExpr, bounded: true, value: multiples.right };
    return tokens.concat([leftToken, rightToken]);
  } else {
    const max: number = difficultyBounds[expr.diff].max;
    const leftToken: SubExprToken = { type: TokenType.SubExpr, bounded: true, value: positiveRandomNum(max) + 1 };
    const rightToken: SubExprToken = { type: TokenType.SubExpr, bounded: true, value: positiveRandomNum(max) + 1 };
    return tokens.concat([leftToken, rightToken]);
  }
};
